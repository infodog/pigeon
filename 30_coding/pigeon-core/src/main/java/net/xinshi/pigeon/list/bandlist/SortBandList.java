...
public class SortBandList implements ISortList {
    ...
    public boolean delete(SortListObject sortObj) throws Exception {
        ...
        synchronized (this) {
            synchronized (((SortBandListFactory) bandService).getGlobalLocker()) {
                di = bandService.writeLogAndDuplicate(OP_DELETE + "," + this.listId + "," + sortObj.getObjid() + "," + sortObj.getKey() + "\n");
                rb = internal_delete(sortObj);
            }
        }
        if (di != null && !di.waitme(1000 * 60)) {
            throw new Exception("delete writeLogAndDuplicate DuplicateService.duplicateData() failed ...... ");
        }
        return rb;
    }
    ...
    public boolean add(SortListObject sortObj) throws Exception {
        ...
        synchronized (this) {
            synchronized (((SortBandListFactory) bandService).getGlobalLocker()) {
                ...
                if(rb){
                    di = bandService.writeLogAndDuplicate(OP_ADD + "," + this.listId + "," + sortObj.getObjid() + "," + sortObj.getKey() + "\n");
                }
            }
        }
        if (di != null && !di.waitme(1000 * 60)) {
            throw new Exception("add writeLogAndDuplicate DuplicateService.duplicateData() failed ...... ");
        }
        return rb;
    }
    ...
    public boolean reorder(SortListObject oldObj, SortListObject newObj) throws Exception {
        ...
        synchronized (this) {
            synchronized (((SortBandListFactory) bandService).getGlobalLocker()) {
                di = bandService.writeLogAndDuplicate(OP_REORDER + "," + this.listId + "," + oldObj.getObjid() + "," + oldObj.getKey() + "," + newObj.getObjid() + "," + newObj.getKey() + "\n");
                rb = internal_reorder(oldObj, newObj);
            }
        }
        if (di != null && !di.waitme(1000 * 60)) {
            throw new Exception("reorder writeLogAndDuplicate DuplicateService.duplicateData() failed ...... ");
        }
        return rb;
    }
