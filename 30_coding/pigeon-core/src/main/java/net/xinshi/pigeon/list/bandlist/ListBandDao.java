...
public class ListBandDao implements IListBandDao {
    ...
    public void insertBand(Band band) throws Exception {
        String sql = String.format("insert into  %s (id,isHead,isMeta,listName,nextMetaBandId,prevMetaBandId,value)values(?,?,?,?,?,?,?)", tableName);
        Connection conn = null;
        try {
            conn = ds.getConnection();
            PreparedStatement pstmt = conn.prepareStatement(sql);
    ...
    public void updateBand(Band band) throws Exception {
        String sql = String.format("update %s set isHead=?,isMeta=?,listName=?,nextMetaBandId=?,prevMetaBandId=?,value=? where id=?", tableName);
        Connection conn = null;
        try {
            conn = ds.getConnection();
            PreparedStatement pstmt = conn.prepareStatement(sql);
