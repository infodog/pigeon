...
public class CommonFlexObjectFactory implements IFlexObjectFactory, IPigeonPersistence {
    ...
    protected String merge_insert_sql = "INSERT INTO T_FLEXOBJECT(NAME, CONTENT, HASH, ISCOMPRESSED, ISSTRING) VALUES (?, ?, ?, ?, ?) " +
            "ON DUPLICATE KEY UPDATE CONTENT=VALUES(CONTENT), HASH=VALUES(HASH), ISCOMPRESSED=VALUES(ISCOMPRESSED), ISSTRING=VALUES(ISSTRING) ";
...
//        }
    ...
    String getFromDB(String name) throws Exception {
        ...
        try {
            ...
            logger.info(sql + " : " + name + " took time " + (end - begin) + "ms");
        }
    }
    ...
    private boolean saveEntries(Collection<FlexObjectEntry> entries) throws Exception {
        ...
        try {
            ...
            if (!merge_insert_db(updateEntries, conn, updateStmt)) {
                return false;
            }
    ...
    private boolean merge_insert_db(List<FlexObjectEntry> updateEntries, Connection conn, PreparedStatement updateStmt) throws Exception {
        try {
            int n = 0;
            long ts = System.currentTimeMillis();
            List<FlexObjectEntry> newEntries = new Vector<FlexObjectEntry>();
            for (FlexObjectEntry entry : updateEntries) {
                updateStmt.setString(1, entry.getName());
                updateStmt.setBlob(2, new ByteArrayInputStream(entry.getBytesContent()));
                updateStmt.setLong(3, entry.getHash());
                updateStmt.setBoolean(4, entry.isCompressed());
                updateStmt.setBoolean(5, entry.isString());
                updateStmt.addBatch();
                ++merge_insert_count;
                if (++n % 1000 == 0) {
                    int[] updateCounts = updateStmt.executeBatch();
                    // System.out.println(TimeTools.getNowTimeString() + " merge_insert_db executeBatch. merge_insert_count = " + merge_insert_count);
                }
            }
            int[] updateCounts = updateStmt.executeBatch();
            long te = System.currentTimeMillis();
            // System.out.println(TimeTools.getNowTimeString() + " merge_insert_db completed records = " + n + " times = " + (te - ts));
            return true;
        } catch (Exception e) {
            e.printStackTrace();
        }
