...
public class Tools {
    ...
    public static void checkKeyLength(String key) throws Exception {
        if (key == null || key.length() > net.xinshi.pigeon.distributed.Constants.DB_KEY_MAX_LENGTH) {
            throw new Exception("key = " + key + " is null or length > " + net.xinshi.pigeon.distributed.Constants.DB_KEY_MAX_LENGTH);
        }
    }

    public static void checkNameLength(String name) throws Exception {
        if (name == null || name.length() > net.xinshi.pigeon.distributed.Constants.DB_NAME_MAX_LENGTH) {
            throw new Exception("name = " + name + " is null or length > " + net.xinshi.pigeon.distributed.Constants.DB_NAME_MAX_LENGTH);
        }
    }
